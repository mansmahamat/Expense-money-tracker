        ...
        <li  className="border-gray-400 flex flex-row mb-2">
            <div className="select-none cursor-pointer bg-white rounded-md flex flex-1 items-center p-4  transition duration-500 ease-in-out transform hover:-translate-y-1 hover:shadow-lg">
              <div className={`flex flex-col rounded-md w-10 h-10 justify-center items-center mr-4 ${props.transaction.amount < 0 ? 'bg-red-500' : 'bg-green-500' } `} >
                  <i className={` fas ${props.transaction.text === "sport" ? 'fas fa-shopping-cart' : props.transaction.category === "course" ? 'fa-shopping-cart' : props.transaction.category === "restaurant" ? 'fa-utensils' : props.transaction.category === "assurance" ? 'fa-file-contract' : props.transaction.category === "nounou" ? 'fa-child' : props.transaction.category === "salaire" ? 'fa-euro-sign' : props.transaction.category === "loyer" ? 'fa-house-user' : '' } `}></i>
              </div>
